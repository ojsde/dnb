 ...
 *
 * @brief Class that converts a Galley to a DNB XML document.
 */

namespace APP\plugins\generic\dnb\filter;
...
class DNBXmlFilter extends NativeExportFilter {
	...
	function getClassName(): string {
		return 'APP\plugins\generic\dnb\filter\DNBXmlFilter';
	}
	...
	function &process(&$pubObject) {
		...
		$version = $versionDao->getCurrentVersion('plugins.generic', $plugin->getPluginSettingsPrefix(), true);
		...
		// Marc 520 abstract
		$abstract = $publication->getLocalizedData('abstract', $galleyLocale);
		if (empty($abstract)) $abstract = $publication->getLocalizedData('abstract', $submission->getData('locale'));
		if (!empty($abstract)) {
		...
		foreach ((array) $authors as $author) {
			...
			$locale = $author->getFamilyName($galleyLocale)?$galleyLocale:$submission->getData('locale');
		...
		foreach ((array) $translators as $translator) {
		    $datafield700 = $this->createDatafieldNode($doc, $recordNode, '700', '1', ' ');
			$locale = $translator->getFamilyName($galleyLocale)?$galleyLocale:$submission->getData('locale');
